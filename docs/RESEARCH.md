...
A **human-in-the-loop draft queue** for X (daily) and Reddit (weekly). Python 3 stdlib. Nothing posts. A landing page exists at `https://williamjxj.github.io/cofounder-ads/` — since 2026-09-18 its buttons book a Cal.com fit call. By 2026-09-15 the engine had queued ~31 drafts and **published zero**. Copy was a 12-body loop. The sibling `platform` dashboard is local-only and not required to run the CLI.
